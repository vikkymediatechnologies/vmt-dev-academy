...
const AdminCourseContentPage = () => {
  ...
  const fetchCourseData = async () => {
    ...
    try {
      ...
      const courseResult = await Promise.race([coursePromise, timeout]) as any;

      if (courseResult.error) {
        console.error("Course fetch error:", courseResult.error);
        throw new Error(`Failed to load course: ${courseResult.error.message}`);
      }

      console.log("✅ Course loaded:", courseResult.data);
      ...
      const modulesResult = await Promise.race([modulesPromise, timeout]) as any;

      if (modulesResult.error) {
        console.error("Modules fetch error:", modulesResult.error);
        // Don't throw - just set empty modules
        setModules([]);
      } else {
        console.log("✅ Modules loaded:", modulesResult.data?.length || 0);
        
        // Sort lessons within each module
        const sortedModules = modulesResult.data?.map((m: any) => ({
          ...m,
